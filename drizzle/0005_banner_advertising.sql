...
CREATE TABLE app.banner_assets (
  ...
  CONSTRAINT banner_hash_size CHECK (octet_length(content_sha256) = 32),
  CONSTRAINT banner_media_allowed CHECK (media_type IN ('image/gif','image/png','image/jpeg','image/webp')),
...
CREATE TABLE app.exclusive_placements (
  ...
  CONSTRAINT placement_time CHECK ((status IN ('active','waiting') AND starts_at IS NOT NULL AND expires_at > starts_at) OR status IN ('suspended','expired'))
);
CREATE INDEX exclusive_placement_game_lookup ON app.exclusive_placements(server_id, expires_at) WHERE status='active';

CREATE FUNCTION api.put_server_banner(uuid, bytea, bytea, bytea, varchar, integer, integer, integer, integer, varchar)
RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
DECLARE sid alias for $1; owner alias for $2; bytes alias for $3; digest alias for $4; mime alias for $5; w alias for $6; h alias for $7; frames alias for $8; duration alias for $9; alt alias for $10;
BEGIN
  ...
  INSERT INTO app.banner_assets(server_id,content,content_sha256,media_type,byte_size,width,height,frame_count,animation_duration_ms,alt_text)
  VALUES(sid,bytes,digest,mime,octet_length(bytes),w,h,frames,duration,btrim(alt))
  ON CONFLICT(server_id) DO UPDATE SET content=EXCLUDED.content,content_sha256=EXCLUDED.content_sha256,media_type=EXCLUDED.media_type,
...
BEGIN
  ...
  VALUES(claim.server_id,banner.id,claim.id,package.duration_days,CASE WHEN active_count<3 THEN 'active' ELSE 'waiting' END,
    clock_timestamp(),clock_timestamp()+make_interval(days=>package.duration_days));
...
CREATE VIEW api.public_banners AS SELECT b.id,b.content,b.media_type FROM app.banner_assets b JOIN app.servers s ON s.id=b.server_id
 WHERE b.moderation_status='approved' AND s.status='active';
...
REVOKE ALL ON api.public_banners FROM PUBLIC,hyperdrive_reader;
REVOKE ALL ON FUNCTION api.put_server_banner(uuid,bytea,bytea,bytea,varchar,integer,integer,integer,integer,varchar) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION api.put_server_banner(uuid,bytea,bytea,bytea,varchar,integer,integer,integer,integer,varchar) TO hyperdrive_reader;
...
GRANT SELECT ON api.public_banners TO hyperdrive_reader;
COMMIT;
