...
import { adPlacementService as defaultAdPlacementService } from './adPlacementService'
import type { AdPlacementItem,AdPlacementService,AdminAccessService, AdminAuthService, BannerReviewItem, BannerReviewService, DonationClaimReviewItem, DonationClaimReviewService, ModerationItem, ModerationService } from './types'
...
type AdminPageProps = {
  ...
  adPlacementService?:AdPlacementService
...
export default function AdminPage({
  ...
  bannerReviewService,
  adPlacementService,
  publicAdsEnabled = siteConfig.exclusiveAdsEnabled,
}: AdminPageProps) {
  ...
  useEffect(() => {
    ...
    accessService.canModerate().then(async (allowed) => {
      ...
      try {
        const [pendingItems, donationResult, pendingBanners,placementResult] = await Promise.all([
        ...
        if (active) {
          ...
          setPlacements(placementResult.placements);setPlacementsUnavailable(placementResult.failed)
          setState('ready')
        }
      } catch {
  ...
  return (
    <main className="admin-shell">
      ...
      {state==='ready'&&<section aria-labelledby="ad-management-heading"><h2 id="ad-management-heading">Ads Management</h2><p>{publicAdsEnabled?'Public sponsored advertising is active. Monitor placement status and inventory here.':'Public sponsored advertising is currently disabled.'}</p>{activeAdPlacementService&&placementsUnavailable&&<p role="status">Advertisement records are unavailable right now.</p>}{activeAdPlacementService&&!placementsUnavailable&&placements.length===0&&<p role="status">There are no advertisement placements.</p>}{activeAdPlacementService&&<><div className="ad-inventory" aria-label="Advertising inventory">{[...new Set(placements.map(item=>item.gameName))].map(game=><p key={game}><strong>{game}:</strong> {placements.filter(item=>item.gameName===game&&item.status==='active').length} of 3 active</p>)}</div><div className="moderation-list" role="list" aria-label="Advertisement placements">{placements.map(item=><article key={item.id} className="moderation-card ad-placement-card" role="listitem"><div><p className="moderation-meta">{item.gameName} | {item.durationDays} days | <span className={`placement-status ${item.status}`}>{item.status}</span></p><h3>{item.serverName}</h3><p>{item.status==='active'&&item.expiresAt?`Ends ${formatDate(item.expiresAt)}`:item.status==='waiting'?`Queued ${formatDate(item.queuedAt)}`:item.status==='expired'?'Placement ended':'Not publicly eligible'}</p><p>{item.impressionCount.toLocaleString()} recorded impressions</p><a href={item.website} target="_blank" rel="noopener noreferrer">Review website <span className="visually-hidden">(opens in a new tab)</span></a></div><div className="moderation-actions">{(item.status==='active'||item.status==='waiting')&&<button aria-label={`Suspend advertisement for ${item.serverName}`} disabled={pendingId===item.id} onClick={(event)=>requestPlacementDecision(event.currentTarget,item,'suspend')}>Suspend</button>}{item.status==='suspended'&&<button aria-label={`Reactivate advertisement for ${item.serverName}`} disabled={pendingId===item.id} onClick={(event)=>requestPlacementDecision(event.currentTarget,item,'reactivate')}>Reactivate</button>}</div></article>)}</div></> }</section>}
