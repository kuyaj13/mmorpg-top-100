...
export function PlayerAuthPanel({ service = playerAuthService, onStatusChange, purpose = 'vote' }: { service?: PlayerAuthService; onStatusChange(status: PlayerAuthStatus): void; purpose?: 'vote' | 'submit' }) {
  ...
  async function authenticate(event: FormEvent<HTMLFormElement>) {
    ...
    try { const submitter = (event.nativeEvent as SubmitEvent).submitter as HTMLButtonElement | null; const registering = submitter?.value === 'register'; update(registering ? await service.register(email, password) : await service.signIn(email, password)); if (registering) setFeedback('Check your inbox and spam folder for the verification link.') }
    catch { setFeedback('Sign-in could not be completed. Check your details and try again.') } finally { setPending(false) }
  }

  async function sendVerificationMessage() { setPending(true); setFeedback(''); try { await service.sendVerification(); setFeedback('A new verification email was sent. Check your inbox and spam folder.') } catch { setFeedback('The verification email could not be sent. Please try again.') } finally { setPending(false) } }
