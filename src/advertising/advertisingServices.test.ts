...
it('loads advertising data only through the authenticated Worker API',async()=>{const fetcher=vi.fn<typeof fetch>().mockResolvedValue(Response.json({ok:true,servers:[],exclusiveServers:[],packages:[],claims:[]}));await requestAdvertisingWorkspace('https://api.mmorpgtop100.com','firebase-id-token',fetcher);expect(fetcher).toHaveBeenCalledWith(new URL('https://api.mmorpgtop100.com/api/advertising/workspace'),{cache:'no-store',headers:{authorization:'Bearer firebase-id-token'}})})
