...
export function BannerUploadForm({ servers, service = bannerUploadService, turnstileSiteKey,kind='free' }: BannerUploadFormProps) {
  const exclusive=kind==='exclusive',width=exclusive?936:468,height=exclusive?120:60,maximumBytes=exclusive?1024*1024:512*1024
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    else if (!allowedTypes.has(file.type) || file.size > maximumBytes) nextErrors.file = `Choose a GIF, PNG, or JPEG image no larger than ${exclusive ? '1 MiB' : '512 KiB'}.`
    ...
    setPending(true)
    const result = await service.upload({ serverId, altText, file, turnstileToken,kind })
...
async function hasRequiredDimensions(file: File,width:number,height:number) {
  try {
