...
export const servers = app.table('servers', {
  ...
  gameVersion: varchar('game_version', { length: 60 }),
  ...
  check('servers_game_version_not_blank', sql`${table.gameVersion} IS NULL OR btrim(${table.gameVersion}) <> ''`),
  check('servers_region_not_blank', sql`${table.region} IS NULL OR btrim(${table.region}) <> ''`),
  check('servers_mode_allowed', sql`${table.mode} IS NULL OR ${table.mode} IN ('PvE', 'PvP', 'RPG')`),
  check('servers_description_not_blank', sql`${table.description} IS NULL OR btrim(${table.description}) <> ''`),
