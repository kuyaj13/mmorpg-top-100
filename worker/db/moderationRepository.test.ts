...
describe('moderation repository', () => {
  ...
  it('labels listing changes and returns authoritative current values',async()=>{const row={id:'change',game_slug:'flyff',game_name:'Flyff',name:'Updated',website:'https://updated.example/',game_version:'v2',region:'Global',mode:'PvE',description:'Updated description',created_at:'2026-09-04T00:00:00Z',current_name:'Current',current_website:'https://current.example/',current_game_version:'v1',current_region:'Asia',current_mode:'PvP',current_description:'Current description'};const database=client([]);vi.mocked(database.query).mockResolvedValueOnce({rows:[]}).mockResolvedValueOnce({rows:[row]});await expect(createModerationRepository(()=>database).listPending()).resolves.toEqual([expect.objectContaining({id:'change',requestType:'change',currentListing:expect.objectContaining({name:'Current',website:'https://current.example/'})})])})
