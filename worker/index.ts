...
import { canonicalReference, detectMediaType, safeHttpsUrl } from './validation'
...
type WorkerEnv = Omit<Env, 'CLAIM_MUTATIONS_ENABLED'> & { TURNSTILE_SECRET: string; CLAIM_MUTATIONS_ENABLED: string }
...
export default {
  async fetch(request, env): Promise<Response> {
    try {
      ...
      if (url.pathname === '/api/advertising/banners' && request.method === 'POST') return corsResponse(request, env, await quarantineBanner(request, env))
...
async function createClaim(request: Request, env: WorkerEnv) {
  ...
  if (!serverId || !['exclusive_7_day', 'exclusive_30_day'].includes(packageCode) || !donorReference) return jsonError('Check the claim details and try again.', 400)

  const server = await env.ADS_DB.prepare('SELECT id FROM servers WHERE id = ? AND owner_uid = ? AND active = 1').bind(serverId, identity.uid).first()
  const adPackage = await env.ADS_DB.prepare('SELECT code FROM ad_packages WHERE code = ? AND active = 1').bind(packageCode).first()
  if (!server || !adPackage) return jsonError('Check the claim details and try again.', 400)
  ...
  try {
    await env.ADS_DB.batch([
      env.ADS_DB.prepare('INSERT INTO donation_claims(id, advertiser_uid, server_id, package_code, donor_reference, created_at) VALUES (?, ?, ?, ?, ?, ?)').bind(claimId, identity.uid, serverId, packageCode, donorReference, now),
      env.ADS_DB.prepare("INSERT INTO donation_audit_events(id, claim_id, actor_uid, action, created_at) VALUES (?, ?, ?, 'submitted', ?)").bind(eventId, claimId, identity.uid, now),
    ])
  } catch {
...
async function quarantineBanner(request: Request, env: WorkerEnv) {
  const identity = await requireIdentity(request, env)
  await requireAppCheck(request, env)
  const contentLength = Number(request.headers.get('content-length'))
  const maximumRequestBytes = Number(env.MAX_BANNER_BYTES) + 20_000
  if (!Number.isFinite(contentLength) || contentLength <= 0 || contentLength > maximumRequestBytes) return jsonError('The banner file is too large.', 413)
  await enforceLimit(env, identity.uid, 'banner', 3, 60 * 60)

  const form = await request.formData()
  await requireTurnstile(stringValue(form.get('turnstileToken'), 2048), request, env)
  const claimId = stringValue(form.get('claimId'), 80)
  const altText = stringValue(form.get('altText'), 180).trim()
  const destinationUrl = safeHttpsUrl(stringValue(form.get('destinationUrl'), 2048))
  const file = form.get('banner')
  if (!(file instanceof File) || file.size === 0 || file.size > Number(env.MAX_BANNER_BYTES) || altText.length < 5 || !destinationUrl) return jsonError('Check the banner details and try again.', 400)

  const claim = await env.ADS_DB.prepare("SELECT server_id FROM donation_claims WHERE id = ? AND advertiser_uid = ? AND status IN ('pending', 'verified')").bind(claimId, identity.uid).first<{ server_id: string }>()
  if (!claim) return jsonError('Check the banner details and try again.', 400)
  const bytes = new Uint8Array(await file.arrayBuffer())
  const mediaType = detectMediaType(bytes)
  if (!mediaType) return jsonError('Use a valid GIF, PNG, JPEG, or WebP banner.', 400)
  const hash = await sha256Hex(bytes)
  const bannerId = crypto.randomUUID()
  await env.ADS_DB.prepare('INSERT INTO banner_assets(id, advertiser_uid, server_id, claim_id, original_bytes, original_media_type, original_sha256, alt_text, destination_url, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .bind(bannerId, identity.uid, claim.server_id, claimId, bytes.buffer, mediaType, hash, altText, destinationUrl, new Date().toISOString()).run()
  return Response.json({ ok: true, bannerId, status: 'quarantined' }, { status: 201, headers: noStoreHeaders() })
}
