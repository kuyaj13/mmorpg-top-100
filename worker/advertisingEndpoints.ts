...
export function createAdvertisingEndpoints(d:Deps){return{
 ...
 async banner(request:Request,bannerId:string){if(request.method!=='GET'||!uuid.test(bannerId))return err('Banner not found.',404);const b=await d.repository.getPublicBanner(bannerId,new URL(request.url).searchParams.get('static')==='1');return b?new Response(b.bytes,{headers:{'content-type':b.mediaType,'x-content-type-options':'nosniff','cache-control':'public, max-age=300','content-security-policy':"default-src 'none'; sandbox"}}):err('Banner not found.',404)},
}}
