...
- Claim mutations remain server-disabled because the current Worker prototype writes D1 while claim history and moderation use PostgreSQL through Data Connect. Enablement requires one authoritative PostgreSQL write path; D1 must not become a second claim authority.
